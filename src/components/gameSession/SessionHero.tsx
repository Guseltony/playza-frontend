  ...
  return (
    <section className="bg-surface-dark rounded-xl pr-1 py-4 overflow-hidden relative">
      ...
      <div className="relative rounded-lg overflow-hidden h-70 md:h-90 mb-6 group">
        ...
        <div className="absolute right-0 top-0 flex gap-2 p-2 text-slate-900 dark:text-white bg-secondary/40 font-bold text-xs md:text-base">
          <p>Entry Fee:</p>
          <span>{formatNaira(entryFee)}</span>
        </div>

        <div className="absolute left-0 top-0 flex items-center gap-2 p-2 text-slate-900 dark:text-white bg-accent/40 font-bold text-xs md:text-base">
          <Users />
        ...
        <div className="absolute bottom-6 left-6 right-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="bg-background-dark/60 backdrop-blur-md px-4 py-2 rounded-lg border border-slate-900/10 dark:border-white/10">
              <p className="text-[10px] uppercase text-slate-600 dark:text-slate-400 font-bold">
                Entry Closes In
              </p>
              <div className="flex gap-2 text-xl font-bold text-slate-900 dark:text-white">
                <span>00</span>
